...
async def post(
    ...
    if not categoria:
        raise HTTPException(
            ...
            detail=f'Categoria {categoria_name} não foi localizada'
            )
    #PARTE DO CENTRO DE TREINAMENTO    
    centro_treinamento = (await db_session.execute(
        select(CentroModel).filter_by(nome=atleta_in.centro_treinamento.nome))
    ).scalars().first()    
        
    if not centro_treinamento:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, 
            detail=f'Centro de treinamento {centro_treinamento_nome} não foi localizada'
            )   
    try:
        ...
        raise HTTPException(
            ...
            detail=f'Ocorreu erro ao inserir dados no banco'
        )   
...
@router.get(
    '/{id}', 
    summary='Consultar a atleta pelo ID', 
    status_code=status.HTTP_200_OK,
    response_model=AtletaOut,
)
...
@router.delete(
    '/{id}', 
    summary='Deletar a atleta pelo ID', 
    status_code=status.HTTP_200_OK
)
